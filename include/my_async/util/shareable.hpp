#ifndef MY_ASYNC_UTIL_SHAREABLE_HPP__
#define MY_ASYNC_UTIL_SHAREABLE_HPP__
...
class Shareable
{
	public:
		...
		void exec_to_all(std::function<void(ShareClass*)> func) noexcept{
			if constexpr(is_enable_shared_from_this<ShareClass>::value == false){
				std::lock_guard<std::mutex> lock(mutex_);
				for(auto s : share_){
					func(s);
				}
...
#endif /* MY_ASYNC_UTIL_SHAREABLE_HPP__ */
